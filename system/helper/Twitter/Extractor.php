...
class Twitter_Extractor extends Twitter_Regex {
  ...
  public function extract() {
    return array(
	  ...
	  'locations_with_indices' => $this->extractLocationsWithIndices(),
  ...
  public function extractLocations() {
    preg_match_all(self::REGEX_LOCATION, $this->tweet, $matches);
    return $matches[3];
  }
  /**
   ...
   * @return  array  The hashtag elements in the tweet.
   */
  public function extractLocationsWithIndices() {
    preg_match_all(self::REGEX_LOCATION, $this->tweet, $matches, PREG_OFFSET_CAPTURE);
    $m = &$matches[3];
    for ($i = 0; $i < count($m); $i++) {
      $m[$i] = array_combine(array('location', 'indices'), $m[$i]);
      # XXX: Fix for PREG_OFFSET_CAPTURE returning byte offsets...
      $start = mb_strlen(substr($this->tweet, 0, $matches[1][$i][1]));
      $start += mb_strlen($matches[1][$i][0]);
      $length = mb_strlen($m[$i]['location']);
      $m[$i]['indices'] = array($start, $start + $length + 1);
    }
    return $m;
  }
