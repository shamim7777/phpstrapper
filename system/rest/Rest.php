...
class Rest{
	...
	public function process() {
		try	{		
			...
			try {
				...
				if(count($this->route[$m[0]])!=0)
				{

					if(count($m)>2){
						
						$field = explode('=',$this->route[$m[0]][$m[1]][':any'])[0];
						$method = $controller->getMethod($m[1]);
						$this->request['params'][$field] = $m[2];	
					}else{
						$field = explode('=',$this->route[$m[0]][':any'])[0];
						$method = $controller->getMethod('get');
						$this->request['params'][$field] = $m[1];	
					}							

				}
				else
				{									
					 	if (strpos($expected, '.json') !== FALSE )
						{ 							
							$method = $controller->getMethod($this->request['method']);
						}	
						else if (strpos($expected, '/') !== FALSE and $m[1]!="" )
						{ 										
							$method = $controller->getMethod($m[1]);
			...
			if(!$method->isStatic()) {
				$controller = $controller->newInstance($this->request,$this->db,$this->template);
	...
	private function getController() {	
        ...
        $expected = $this->request['resource'];	
	    $expected = str_replace('api/', '', $expected);
	    
        $url = $this->request['resource'];
		
		if (strpos($expected, '/') !== FALSE)
		{
			$this->request['function']=explode('/',$expected);	
			$expected = explode('/',$expected)[0]; 	
			
		}
		else if(strpos($expected, '.json') !== FALSE)
		{
			$uri = explode('/',$expected)[0]; 
			if($uri=="")
				$expected =	explode('/',$expected)[1]; 
			else
				$expected =explode('.',$expected)[0];
		}{
 ...
 */
abstract class RestController extends Generic {
	protected $request;
	protected $response;
	...
	public function __construct($request,$db,$template) {
